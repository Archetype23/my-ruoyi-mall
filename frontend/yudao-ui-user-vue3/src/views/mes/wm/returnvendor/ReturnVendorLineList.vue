...
const list = ref<WmReturnVendorLineVO[]>([]) // 行列表
...
const handleDelete = async (id: number) => {
  try {
    ...
    await WmReturnVendorLineApi.deleteReturnVendorLine(id)
...
const openForm = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await WmReturnVendorLineApi.getReturnVendorLine(id)
    } finally {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (lineFormType.value === 'create') {
      ...
      await WmReturnVendorLineApi.updateReturnVendorLine(data)
...
// ==================== 展开行：拣货明细 ====================
const detailListRefs = ref<Record<number, InstanceType<typeof ReturnVendorDetailList>>>({})
