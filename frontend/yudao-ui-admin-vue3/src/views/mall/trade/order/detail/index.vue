  ...
  <ContentWrap v-if="formData.id">
    ...
    <el-descriptions title="订单信息">
      ...
      <el-descriptions-item label="订单类型: ">
        <el-tag v-if="isCommunityGroupBuy" type="success">社区拼团订单</el-tag>
        <dict-tag v-else :type="DICT_TYPE.TRADE_ORDER_TYPE" :value="formData.type!" />
    ...
    <el-descriptions :column="1" title="订单状态">
      <el-descriptions-item v-if="isCommunityGroupBuy" label="社区拼团: ">
        该订单来自西北师大社区团购。团单成团后可发货；未成团时后端会阻止发货操作。
      </el-descriptions-item>
...
const isCommunityGroupBuy = computed(() => formData.value.type === 5)
...
/** 复制地址 */
const onCopyAddress = () => {
  const text = (formData.value.receiverAreaName || '') + ' ' + (formData.value.receiverDetailAddress || '')
  if (text.trim()) {
...
/** 初始化 **/
const deliveryExpressList = ref<any[]>([]) // 物流公司
const expressTrackList = ref<any[]>([]) // 物流详情
const pickUpStore = ref<any>({}) // 自提门店
onMounted(async () => {
  await getDetail()
  // 如果配送方式为快递，则查询物流公司
