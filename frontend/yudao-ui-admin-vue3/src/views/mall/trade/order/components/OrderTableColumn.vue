        ...
        <el-table-column min-width="300" prop="spuName">
          <template #header>
            ...
            >
              ...
              <span>订单类型：</span>
              <el-tag v-if="isCommunityGroupBuy(scope.row)" type="success">社区拼团订单</el-tag>
              <dict-tag v-else :type="DICT_TYPE.TRADE_ORDER_TYPE" :value="scope.row.type" />
              <span v-if="isCommunityGroupBuy(scope.row)" class="ml-12px text-green-600">
                成团后发货
              </span>
            </div>
          </template>
...
const isCommunityGroupBuy = (order: OrderVO) => order.type === 5

const headerStyle: any = ({ row, columnIndex }: any) => {
  // 表头第一行第一列占 8
  if (columnIndex === 0) {
