...
// 延迟加载重依赖，避免 api -> request -> platform/store/api 循环初始化。
const getStore = (name) => import('@/sheep/store').then((m) => m.default(name));
const getPlatform = () => import('@/sheep/platform').then((m) => m.default);
const openAuthModal = () => import('@/sheep/hooks/useModal').then((m) => m.showAuthModal());
...
http.interceptors.request.use(
  async (config) => {
    ...
    config.header['platform'] = (await getPlatform()).name;
