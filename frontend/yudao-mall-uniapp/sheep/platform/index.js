...
const getUni = () => (typeof uni === 'undefined' ? undefined : uni);
const getDevice = () => {
  const uniApi = getUni();
...
const getOs = () => getUni()?.getDeviceInfo?.().platform || 'web';
...
if (!device.safeAreaInsets) {
  device.safeAreaInsets = getUni()?.getSystemInfoSync?.().safeAreaInsets
}
// 兜底一下。还是没有值时候，就给个默认值
if (!device.safeAreaInsets) {
...
// 支付服务转发
const pay = async (payment, orderType, orderSN) => {
  // pay.js imports the sheep aggregate. Load it only when payment starts to avoid
  // sheep/index -> platform -> pay -> sheep/index circular initialization.
  const { default: Pay } = await import('./pay');
...
async function checkNetwork() {
  const uniApi = getUni();
  if (!uniApi?.getNetworkType) {
    return Promise.resolve(true);
  }
  const networkStatus = await uniApi.getNetworkType();
...
const getCapsule = () => {
  // #ifdef MP
  let capsule = getUni()?.getMenuButtonBoundingClientRect?.();
  if (!capsule) {
    capsule = {
      bottom: 56,
      height: 32,
