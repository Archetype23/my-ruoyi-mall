<!-- 支付结果页面 -->
<template>
  <s-layout :bgStyle="{ color: '#FFF' }" title="支付结果" navbar="custom" :showLeftButton="true">
    <view class="pay-result-box ss-flex-col ss-row-center ss-col-center">
      ...
      />
      <view class="tip-text ss-m-b-30" v-if="payResult === 'success'">{{ successTitle }}</view>
      ...
      <view class="group-buy-tip" v-if="payResult === 'success' && isGroupBuyOrder">
        <image class="tip-logo" src="/static/logo.png" mode="aspectFit" />
        <view>
          <text class="tip-title">已加入西北师大社区拼团</text>
          <text class="tip-desc">可在“我的拼团”查看成团进度，成团后商家按订单发货。</text>
        </view>
      </view>
      ...
      <view class="btn-box ss-flex ss-row-center ss-m-t-50">
        ...
        <button
          class="check-btn ss-reset-button group-buy-btn"
          v-if="payResult === 'success' && isGroupBuyOrder"
          @tap="sheep.$router.redirect('/pages/groupbuy/mine')"
        >
          我的拼团
        </button>
        <button
...
<script setup>
  ...
  const isGroupBuyOrder = computed(() => state.tradeOrder.type === 5);
  const successTitle = computed(() => (isGroupBuyOrder.value ? '拼团支付成功' : '支付成功'));
