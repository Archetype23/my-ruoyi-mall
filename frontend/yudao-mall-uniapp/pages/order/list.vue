<!-- 订单列表 -->
<template>
  <s-layout title="我的订单" navbar="custom" :showLeftButton="true">
      ...
      >
        <view class="order-card-header ss-flex ss-col-center ss-row-between ss-p-x-20">
          <view class="order-title-block">
            ...
            <view v-if="isGroupBuyOrder(order)" class="group-buy-label">西北师大社区拼团</view>
          </view>
        ...
        >
          <view class="ss-flex ss-col-center">
            <button
              v-if="order.buttons.includes('groupBuy')"
              class="tool-btn group-buy-tool ss-reset-button"
              @tap.stop="onOrderGroupBuy(order)"
            >
              拼团进度
            </button>
            <button
...
<script setup>
  ...
  // 跳转社区拼团记录
  function onOrderGroupBuy(order) {
    if (order.groupBuyHeadId) {
      sheep.$router.go('/pages/groupbuy/head', {
        id: order.groupBuyHeadId,
      });
      return;
    }
  ...
  function isGroupBuyOrder(order) {
    return order.type === 5;
  }
