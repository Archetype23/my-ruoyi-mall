<template>
  <s-layout title="确认订单" navbar="custom" :showLeftButton="true">
    <!-- 头部地址选择【配送地址】【自提地址】 -->
    <AddressSelection v-model="addressState" />

    <view v-if="isCommunityGroupBuy" class="campus-group-order">
      <view class="campus-group-head">
        <view class="campus-group-brand">
          <image class="campus-group-logo" src="/static/logo.png" mode="aspectFit" />
          <view>
            <view class="campus-group-eyebrow">西北师大社区团购</view>
            <view class="campus-group-title">社区拼团确认订单</view>
          </view>
        </view>
        <text class="campus-group-tag">成团后发货</text>
      </view>
      <view class="campus-group-desc">
        本单将按拼团价结算。支付后可在“我的拼团”查看成团进度，团单成团后由商家统一发货。
      </view>
      <view class="campus-group-steps">
        <text>提交订单</text>
        <text>支付参团</text>
        <text>等待成团</text>
        <text>商家发货</text>
    ...
    <su-fixed bottom :opacity="false" bg="bg-white" placeholder :noFixed="false" :index="200">
      <view class="footer-box border-top ss-flex ss-row-between ss-p-x-20 ss-col-center">
        ...
        <button
          ...
          @tap="onConfirm"
        >
          {{ isCommunityGroupBuy ? '支付并参团' : '提交订单' }}
        </button>
      </view>
    </su-fixed>
  </s-layout>
...
<script setup>
  ...
  const isCommunityGroupBuy = computed(() => Number(state.orderPayload.groupBuyActivityId || 0) > 0);
  ...
  async function submitOrder() {
    const { code, data } = await OrderApi.createOrder({
      ...
      groupBuyActivityId: state.orderPayload.groupBuyActivityId,
      groupBuyHeadId: state.orderPayload.groupBuyHeadId,
...
<style lang="scss" scoped>
  ...
  .campus-group-order {
    ...
    border-radius: 8rpx;
    background: #ffffff;
    box-shadow: 0 10rpx 26rpx rgba(243, 123, 29, 0.08);
  }
