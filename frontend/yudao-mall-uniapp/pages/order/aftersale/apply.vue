...
<template>
  <s-layout title="申请售后">
    ...
    <view v-if="isCommunityGroupBuy" class="campus-after-sale">
      <view class="campus-title">西北师大社区拼团售后</view>
      <view class="campus-desc">
        如团单未成团，系统会按拼团规则处理退款；如已成团发货，请按实际收货情况提交售后申请。
      </view>
    </view>
...
<script setup>
  ...
  const isCommunityGroupBuy = computed(() => state.order.type === 5);
  ...
  onLoad(async (options) => {
    ...
    state.reasonList = state.config.afterSaleRefundReasons || [];
  });
</script>
...
<style lang="scss" scoped>
  .campus-after-sale {
    ...
    border-radius: 8rpx;
    background: #ffffff;
    box-shadow: 0 10rpx 26rpx rgba(24, 72, 55, 0.08);
  }
