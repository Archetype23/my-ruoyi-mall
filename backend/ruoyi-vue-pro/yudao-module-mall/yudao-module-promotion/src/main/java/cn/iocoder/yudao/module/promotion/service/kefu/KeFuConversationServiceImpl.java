...
public class KeFuConversationServiceImpl implements KeFuConversationService {
    ...
    @Override
    public List<KeFuConversationDO> getKefuConversationList(Long deptId, Long adminId) {
        return conversationMapper.selectConversationList(deptId, adminId);
    }
    ...
    public KeFuConversationDO getOrCreateConversation(Long userId) {
        ...
        if (conversation == null) {
            Long deptId = getDefaultDeptId();
            // 轮询：分配给当前有效会话最少的客服
            Long nextKefu = conversationMapper.selectNextKefu(deptId);
    ...
    @Override
    public void updateConversationAssignedTo(Long id, Long adminId) {
        conversationMapper.updateById(new KeFuConversationDO().setId(id).setAssignedTo(adminId));
    }
