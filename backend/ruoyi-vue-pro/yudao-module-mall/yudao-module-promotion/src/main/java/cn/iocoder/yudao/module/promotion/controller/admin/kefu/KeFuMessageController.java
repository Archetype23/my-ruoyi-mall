...
public class KeFuMessageController {
    ...
    public CommonResult<Long> sendKeFuMessage(@Valid @RequestBody KeFuMessageSendReqVO sendReqVO) {
        sendReqVO.setSenderId(getLoginUserId()).setSenderType(UserTypeEnum.ADMIN.getValue());
        // 自动分配：如果会话未分配客服，则分配给当前回复的客服
        ...
        if (conversation.getAssignedTo() == null) {
            conversationService.updateConversationAssignedTo(conversation.getId(), getLoginUserId());
        }
    ...
    public CommonResult<List<KeFuMessageRespVO>> getKeFuMessageList(@Valid KeFuMessageListReqVO pageReqVO) {
        ...
        // 会员头像
        Map<Long, MemberUserRespDTO> memberMap = memberUserApi.getUserMap(convertSet(filterList(result,
                item -> UserTypeEnum.MEMBER.getValue().equals(item.getSenderType())), KeFuMessageRespVO::getSenderId));
        result.forEach(item -> findAndThen(memberMap, item.getSenderId(), member -> item.setSenderAvatar(member.getAvatar())));
