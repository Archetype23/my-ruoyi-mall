...
public class MemberStatisticsController {
    ...
    public CommonResult<MemberAnalyseRespVO> getMemberAnalyse(MemberAnalyseReqVO reqVO) {
        ...
        if (times == null || times.length < 2) {
            times = new LocalDateTime[]{LocalDateTime.now().minusDays(30), LocalDateTime.now()};
        }
        LocalDateTime beginTime = ArrayUtil.get(times, 0);
        LocalDateTime endTime = ArrayUtil.get(times, 1);
        // 1.1 查询分析对照数据
