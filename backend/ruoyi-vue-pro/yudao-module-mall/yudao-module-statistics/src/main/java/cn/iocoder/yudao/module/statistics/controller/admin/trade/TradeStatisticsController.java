...
public class TradeStatisticsController {
    ...
    public CommonResult<DataComparisonRespVO<TradeTrendSummaryRespVO>> getTradeStatisticsAnalyse(TradeTrendReqVO reqVO) {
        ...
        if (times == null || times.length < 2) {
            times = new LocalDateTime[]{LocalDateTimeUtil.now().minusDays(30), LocalDateTimeUtil.now()};
        }
        return success(tradeStatisticsService.getTradeStatisticsAnalyse(times[0], times[1]));
    }
    ...
    public CommonResult<List<TradeTrendSummaryRespVO>> getTradeStatisticsList(TradeTrendReqVO reqVO) {
        ...
        List<TradeStatisticsDO> list = tradeStatisticsService.getTradeStatisticsList(times[0], times[1]);
