...
public class AppTradeOrderSettlementReqVO {
    ...
    // ========== 社区拼团活动相关字段 ==========
    @Schema(description = "社区拼团活动编号", example = "1024")
    private Long groupBuyActivityId;

    @Schema(description = "社区拼团团单编号", example = "2048")
    private Long groupBuyHeadId;
    ...
    public boolean isValidActivityItems() {
        ...
        if (ObjUtil.isAllEmpty(seckillActivityId, combinationActivityId, combinationHeadId,
                groupBuyActivityId, groupBuyHeadId, bargainRecordId)) {
            return true;
        }
        // 校验订单项是否超出
        return items.size() == 1;
