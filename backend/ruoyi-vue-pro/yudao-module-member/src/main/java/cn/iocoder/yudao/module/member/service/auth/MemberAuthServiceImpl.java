...
public class MemberAuthServiceImpl implements MemberAuthService {
    ...
    public AppAuthLoginRespVO register(AppAuthRegisterReqVO reqVO) {
        // 校验手机号未被注册
        MemberUserDO existingUser = userService.getUserByMobile(reqVO.getMobile());
        if (existingUser != null) {
            throw exception(AUTH_REGISTER_MOBILE_EXISTS);
        }
        // 创建用户（createUserIfAbsent 会创建随机密码的用户）
        String userIp = getClientIP();
        MemberUserDO user = userService.createUserIfAbsent(reqVO.getMobile(), userIp, getTerminal());
        Assert.notNull(user, "注册用户失败，结果为空");
        // 设置用户密码
        userService.setUserPassword(user.getId(), reqVO.getPassword());
        // 自动登录
